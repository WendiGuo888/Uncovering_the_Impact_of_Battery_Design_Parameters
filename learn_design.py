...
def extract_voltage_features(voltage_series):
    # Extract various statistical features from a pandas series of voltage data
    features = {
...
def process_and_analyze_battery_data(file_path, variable_name, voltage_start_threshold, voltage_end_threshold):
    ...
    # plt.scatter(y_test, y_pred_important, edgecolor='k', alpha=0.7,label=f'Test MSE: {mse_important:.2f}')
    # plt.plot([min(y_test), max(y_test)], [min(y_test), max(y_test)], 'r--')
    # plt.xlabel('Actual (log_' + variable_name + ')', fontsize = 14)
    # plt.ylabel('Predicted (log_' + variable_name + ')', fontsize = 14)
    # plt.show()
