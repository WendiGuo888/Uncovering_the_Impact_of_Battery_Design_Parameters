...
#%% data spliting, physical feautre addition, feature selection and nomalization
def prepare_battery_dataset(
    ...
    variable_names,
    target_col_indices,
    important_physical_features,
    train_ratio=0.8,
    seed=42,
    include_physical_features=True,
    aging_columns=None,
    return_dataframe=False  # If True, return X/y as DataFrames
):
    """
    Encapsulate the full pipeline of data splitting, physical feature addition, feature selection and normalization, and X/y assembly.
    """
    ...
    processed_train = process_features_with_physics(
        train_dataframes, result_dictionaries, variable_names, include_physical_features
    )
    processed_test = process_features_with_physics(
        test_dataframes, result_dictionaries, variable_names, include_physical_features
    )

    # feature selection and normalization
    train_selected_data, test_selected_data, selected_features, scaler = process_train_val_test_data_optimized(
        train_dataframes=processed_train,
        test_dataframes=processed_test,
        target_col_indices=target_col_indices,
        cor_limit=16,
        important_physical_features=important_physical_features,
        random_state=42,
        aging_columns=aging_columns
    )

    if return_dataframe:
        X_train = pd.concat([d['X'] for d in train_selected_data['train'].values()])
        y_train = pd.concat([d['y'] for d in train_selected_data['train'].values()])
        X_test = pd.concat([d['X'] for d in test_selected_data['test'].values()])
        y_test = pd.concat([d['y'] for d in test_selected_data['test'].values()])
    else:
        X_train = pd.concat([d['X'] for d in train_selected_data['train'].values()]).values
        y_train = pd.concat([d['y'] for d in train_selected_data['train'].values()]).values
        X_test = pd.concat([d['X'] for d in test_selected_data['test'].values()]).values
        y_test = pd.concat([d['y'] for d in test_selected_data['test'].values()]).values
